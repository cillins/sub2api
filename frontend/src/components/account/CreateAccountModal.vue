    ...
    >
      ...
      <div>
        ...
        <div class="mt-2 flex rounded-lg bg-gray-100 p-1 dark:bg-dark-700" data-tour="account-form-platform">
          ...
          <button
            type="button"
            @click="form.platform = 'mulerun'"
            :class="[
              'flex flex-1 items-center justify-center gap-2 rounded-md px-4 py-2.5 text-sm font-medium transition-all',
              form.platform === 'mulerun'
                ? 'bg-white text-gray-800 shadow-sm dark:bg-dark-600 dark:text-gray-200'
                : 'text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-gray-200'
            ]"
          >
            <PlatformIcon platform="mulerun" size="sm" />
            MuleRun
          </button>
        </div>
      </div>
      ...
      <!-- MuleRun API Key input + Model Restriction -->
      <div v-if="form.platform === 'mulerun'" class="space-y-4">
        <div>
          <label class="input-label">{{ t('admin.accounts.apiKey') }}</label>
          <input
            v-model="mulerunApiKey"
            type="password"
            required
            class="input font-mono"
            placeholder="muk-..."
          />
          <p class="input-hint">{{ t('admin.accounts.types.mulerunApikey') }}</p>
        </div>
        <div>
          <label class="input-label">{{ t('admin.accounts.baseUrl') }}</label>
          <input
            v-model="mulerunBaseUrl"
            type="text"
            class="input"
            placeholder="https://api.mulerun.com"
          />
          <p class="input-hint">{{ t('admin.accounts.mulerun.baseUrlHint') }}</p>
        </div>

        <!-- Model Restriction Section (MuleRun) -->
          ...
          <div v-else>
            ...
            <div v-if="modelMappings.length > 0" class="mb-3 space-y-2">
              <div v-for="(mapping, index) in modelMappings" :key="getModelMappingKey(mapping)" class="flex items-center gap-2">
                ...
                <button type="button" @click="removeModelMapping(index)" class="rounded-lg p-2 text-red-500 transition-colors hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-900/20">
      ...
      <!-- API Key input (only for apikey type, excluding Antigravity and MuleRun which have their own fields) -->
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity' && form.platform !== 'mulerun'" class="space-y-4">
        <div>
          <label class="input-label">{{ t('admin.accounts.baseUrl') }}</label>
          <input
...
const mulerunApiKey = ref('') // For MuleRun platform: API key
const mulerunBaseUrl = ref('') // For MuleRun platform: optional base URL override
...
const isOAuthFlow = computed(() => {
  ...
  // MuleRun 不需要 OAuth 流程
  if (form.platform === 'mulerun') {
    return false
  }
...
watch(
  ...
  (newPlatform) => {
    ...
    // Reset MuleRun fields when switching platforms
    mulerunApiKey.value = ''
    mulerunBaseUrl.value = ''
...
const handleSubmit = async () => {
  ...
  // For MuleRun platform, create directly as apikey
  if (form.platform === 'mulerun') {
    if (!form.name.trim()) {
      appStore.showError(t('admin.accounts.pleaseEnterAccountName'))
      return
    }
    if (!mulerunApiKey.value.trim()) {
      appStore.showError(t('admin.accounts.mulerun.apiKeyRequired'))
      return
    }

    const credentials: Record<string, unknown> = {
      api_key: mulerunApiKey.value.trim()
    }
    if (mulerunBaseUrl.value.trim()) {
      credentials.base_url = mulerunBaseUrl.value.trim()
    }

    // Apply model restriction (whitelist or mapping)
    const mulerunModelMapping = buildModelMappingObject(
      modelRestrictionMode.value, allowedModels.value, modelMappings.value
    )
    if (mulerunModelMapping) {
      credentials.model_mapping = mulerunModelMapping
    }
